...
function TPsetupAdminAreas()
{
	...
	$context['admin_tabs']['custom_modules'] = [];
...
function TP_addPerms()
{
	...
	call_integration_hook('integrate_tp_admin_permissions', [&$admperms]);
...
function TPcollectPermissions()
{
	...
	// for SMF2
	$context['TPortal']['tppermissonlist'] = [
		'tp_settings' => [false, 'tinyportal', 'tinyportal'],
		'tp_blocks' => [false, 'tinyportal', 'tinyportal'],
		'tp_articles' => [false, 'tinyportal', 'tinyportal'],
		'tp_submithtml' => [false, 'tinyportal', 'tinyportal'],
		'tp_submitbbc' => [false, 'tinyportal', 'tinyportal'],
		'tp_editownarticle' => [false, 'tinyportal', 'tinyportal'],
		'tp_alwaysapproved' => [false, 'tinyportal', 'tinyportal'],
		'tp_dlmanager' => [false, 'tinyportal', 'tinyportal'],
		'tp_dlupload' => [false, 'tinyportal', 'tinyportal']
	];

	$context['TPortal']['adminlist'] = [
		'tp_settings' => 1,
		'tp_blocks' => 1,
		'tp_articles' => 1,
		'tp_dlmanager' => 1,
...
function tp_getbuttons()
{
	...
	if (allowedTo('tp_submithtml') || allowedTo('tp_articles')) {
		$buts['tpeditwonarticle']['sub_buttons']['submithtml'] = [
	...
	if (allowedTo('tp_submitbbc') || allowedTo('tp_articles')) {
		$buts['tpeditwonarticle']['sub_buttons']['submitbbc'] = [
	...
	if ($handle = opendir($context['TPortal']['blockcode_upload_path'])) {
		while (false !== ($file = readdir($handle))) {
			if ($file != '.' && $file != '..' && $file != '.htaccess' && substr($file, (strlen($file) - 10), 10) == '.blockcode') {
				$snippet = TPparseModfile(file_get_contents($context['TPortal']['blockcode_upload_path'] . $file), ['name', 'author', 'version', 'date', 'description']);
				$codefiles[] = [
...
function TP_article_categories($use_sorted = false)
{
	...
	$context['TPortal']['categories_shortname'] = [];
	...
	$context['TPortal']['article_categories'] = [];
	if ($use_sorted) {
		// sort them
		if (count($sorted) > 1) {
...
function tp_cleantitle($text)
{
	...
	$tmp = strtr($tmp, ['Þ' => 'TH', 'þ' => 'th', 'Ð' => 'DH', 'ð' => 'dh', 'ß' => 'ss', '' => 'OE', '' => 'oe', 'Æ' => 'AE', 'æ' => 'ae', 'µ' => 'u']);
	$cleaned = preg_replace(['/\s/', '/[^\w_\.\-]/'], ['_', ''], $tmp);
	return $cleaned;
}
...
function TP_permaTheme($theme)
{
	...
	if (isset($context['TPortal']['querystring'])) {
		$tp_where = str_replace([';permanent'], [''], $context['TPortal']['querystring']);
	}
	else {
...
function TP_setThemeLayer($layer, $template, $subtemplate, $admin = false)
{
	...
	if ($admin) {
		...
		if (file_exists($settings['theme_dir'] . '/' . $template . '.css')) {
			$context['html_headers'] .= '<link rel="stylesheet" type="text/css" href="' . $settings['theme_url'] . '/' . $template . '.css?' . TPVERSION . '" />';
		}
		else {
			$context['html_headers'] .= '<link rel="stylesheet" type="text/css" href="' . $settings['default_theme_url'] . '/' . $template . '.css?' . TPVERSION . '" />';
		}
...
function TPwysiwyg_setup()
{
	...
	$context['html_headers'] .= '
		<link rel="stylesheet" href="' . $settings['default_theme_url'] . '/scripts/tinyportal/sceditor/minified/themes/default.min.css" />
		<script src="' . $settings['default_theme_url'] . '/scripts/tinyportal/sceditor/minified/sceditor.min.js"></script>
		<script src="' . $settings['default_theme_url'] . '/scripts/tinyportal/sceditor/minified/formats/xhtml.js"></script>
		<script src="' . $settings['default_theme_url'] . '/scripts/tinyportal/sceditor/languages/' . $txt['lang_dictionary'] . '.js"></script>
		<style>
			.sceditor-button-floatleft div { background: url(' . $settings['default_theme_url'] . '/images/tinyportal/floatleft.png); width:24px; height:24px; margin: -3px; }
			.sceditor-button-floatright div { background: url(' . $settings['default_theme_url'] . '/images/tinyportal/floatright.png); width:24px; height:24px; margin: -3px; }
		</style>';
...
function TPwysiwyg($textarea, $body, $upload = true, $uploadname = false, $use = 1, $showchoice = true)
{
			...
			var dragdropOptions = {
			    ...
			    handleFile: function (file, createPlaceholder) {
					...
					tpImageUpload(file).then(function (url) {
						...
						placeholder.insert(\'<img src="\' + url + \'" alt="" />\');
					}).catch(function () {
	...
	echo '
				...
				style: "' . $settings['default_theme_url'] . '/scripts/tinyportal/sceditor/minified/themes/content/default.min.css",
				emoticonsRoot: "' . $settings['default_theme_url'] . '/scripts/tinyportal/sceditor/"
			});
		</script>';
	...
	if ($smcFunc['db_num_rows']($request) > 0) {
		while ($row = $smcFunc['db_fetch_assoc']($request)) {
			if ($row['value5'] != -1 && $row['value2'] != '-1') {
				...
				if (in_array($mtype, ['cats', 'arti', 'link'])) {
					$menu[] = [
						'id' => $row['id'],
						'name' => $row['value1'],
						'pos' => $menupos,
...
function tp_fetchboards()
{
	...
	if ($smcFunc['db_num_rows']($request) > 0) {
		while ($row = $smcFunc['db_fetch_assoc']($request)) {
			$boards[] = ['id' => $row['id_board'], 'name' => $row['name']];
		}
...
function tp_collectArticleAttached($art)
{
	...
	// get attached images
	$context['TPortal']['illustrations'] = [];
	$context['TPortal']['illustrations_align'] = [];
	$context['TPortal']['illustrations_text'] = [];
...
function TP_fetchprofile_areas()
{
	...
	$areas = [
		'tp_summary' => ['name' => 'tp_summary', 'permission' => 'profile_view_any'],
		'tp_articles' => ['name' => 'tp_articles', 'permission' => 'tp_articles'],
		'tp_download' => ['name' => 'tp_download', 'permission' => 'tp_dlmanager'],
	];

	call_integration_hook('integrate_tp_profile_areas', [&$areas]);
...
function TP_fetchprofile_areas2($member_id)
{
	...
	if (!$user_info['is_guest'] && (($context['user']['is_owner'] && allowedTo('profile_view_own')) || allowedTo(['profile_view_any', 'moderate_forum', 'manage_permissions', 'tp_dlmanager', 'tp_blocks', 'tp_articles', 'tp_gallery', 'tp_linkmanager']))) {
		$context['profile_areas']['tinyportal'] = [
		...
		call_integration_hook('integrate_tp_profile', [&$member_id]);
	}
}
...
function TPstrip_linktree()
{
	...
	$context['linktree'] = [];
	$context['linktree'][] = ['url' => $scripturl, 'name' => $context['forum_name']];
}
...
function TPageIndex($base_url, &$start, $max_value, $num_per_page)
{
	...
	$base_link = '<a class="navPages" href="' . ($flexible_start ? $base_url : strtr($base_url, ['%' => '%%']) . ';p=%d') . '">%s</a> ';
...
function render_template_layout($code, $prefix = '')
{
	...
	if (!empty($context['TPortal']['disable_template_eval'])) {
		...
		$ncode = 'echo \'' . str_replace(['{', '}'], ["', " . $prefix, "(), '"], $code) . '\';';
		eval($ncode);
	}
}
...
function category_col($column, $featured = false, $render = true)
{
	...
	if ($column == 'featured') {
		$context['TPortal']['category']['featured'] = [$context['TPortal']['category']['featured']];
	}
...
function TPadminIndex($tpsub = '', $module_admin = false)
{
	...
	if (allowedTo('tp_editownarticle')) {
		$context['admin_tabs']['tp_articles'] = [
			'myarticles' => [
				'title' => $txt['tp-myarticles'],
	...
	if (allowedTo('tp_articles')) {
		$context['admin_tabs']['tp_articles'] = [
			'articles' => [
				...
				'description' => $txt['tp-articledesc1'],
				'href' => $scripturl . '?action=tpadmin;sa=articles',
				'is_selected' => (substr($tpsub, 0, 11) == 'editarticle' || in_array($tpsub, ['articles', 'addarticle', 'addarticle_php', 'addarticle_bbc', 'addarticle_import', 'strays', 'submission'])),
			],
			'categories' => [
				'title' => $txt['tp-tabs5'],
				'description' => $txt['tp-articledesc2'],
				'href' => $scripturl . '?action=tpadmin;sa=categories',
				'is_selected' => in_array($tpsub, ['categories', 'addcategory', 'clist']),
			],
			'artsettings' => [
...
function tp_collectArticleIcons()
{
	...
	$count = 1;
	$context['TPortal']['articons'] = [];
	$context['TPortal']['articons']['illustrations'] = [];

	$sorted2 = [];
	//illustrations/images
	if ($handle = opendir($boarddir . '/tp-files/tp-articles/illustrations')) {
		while (false !== ($file = readdir($handle))) {
			if ($file != '.' && $file != '..' && $file != '.htaccess' && $file != 'TPno_illustration.png' && in_array(strtolower(substr($file, strlen($file) - 4, 4)), ['.gif', '.jpg', '.png'])) {
...
function tp_recordevent($date, $id_member, $textvariable, $link, $description, $allowed, $eventid)
{
	...
	$smcFunc['db_insert'](
		...
		[$id_member, $date, $textvariable, $link, $description, $allowed, $eventid, 0],
		['id']
	);
}
...
function tpattach()
{
	...
	ob_end_clean();
	if (!empty($modSettings['enableCompressedOutput']) && @version_compare(PHP_VERSION, '4.2.0') >= 0 && @filesize($filename) <= 4194304 && in_array($file_ext, ['txt', 'html', 'htm', 'js', 'doc', 'pdf', 'docx', 'rtf', 'css', 'php', 'log', 'xml', 'sql', 'c', 'java'])) {
	...
	elseif (filesize($filename) != 0) {
		...
		if (!empty($size)) {
			...
			if (!empty($size['mime']) && !in_array($size[2], [4, 13])) {
				header('Content-Type: ' . strtr($size['mime'], ['image/bmp' => 'image/x-ms-bmp']));
			}
	...
	if (!isset($_REQUEST['image']) && in_array($file_ext, ['gif', 'jpg', 'bmp', 'png', 'jpeg', 'tiff'])) {
	...
	if (is_countable($mycats) && count($mycats) > 0) {
		...
		if ($sort == 'weekdownloads') {
			$request = $smcFunc['db_query'](
				...
				['type' => 'dlitem', 'cat' => $mycats, 'sort' => $sortstring, 'limit' => $number]
			);
		}
		else {
			$request = $smcFunc['db_query'](
...
function dl_getcats()
{
	...
	$context['TPortal']['dl_allowed_cats'] = [];
	$request = $smcFunc['db_query'](
		'',
		'
	...
	if ($smcFunc['db_num_rows']($request) > 0) {
		while ($row = $smcFunc['db_fetch_assoc']($request)) {
			...
			if ($show) {
				$context['TPortal']['dl_allowed_cats'][$row['id']] = [
					'id' => $row['id'],
					'name' => $row['name'],
					'parent' => $row['parent'],
				];
...
function tp_getDLcats()
{
	...
	if ($smcFunc['db_num_rows']($request) > 0) {
		while ($row = $smcFunc['db_fetch_assoc']($request)) {
			$context['TPortal']['dlcats'][$count] = ['id' => $row['id'], 'name' => $row['name']];
			$count++;
		}
		$smcFunc['db_free_result']($request);
	}
...
function TPGetMemberColour($member_ids)
{
	...
	if (empty($db_connection)) {
		...
		$options = array_merge($db_options, ['persist' => $db_persist, 'dont_select_db' => SMF == 'SSI']);
...
function tp_profile_summary($memID)
{
	...
	if ($context['TPortal']['show_download']) {
		...
		$request = $smcFunc['db_query'](
			...
			['author' => $memID, 'type' => 'dlitem']
		);
		$result = $smcFunc['db_fetch_row']($request);
		$max_upload = $result[0];
		$smcFunc['db_free_result']($request);
	}
	$context['TPortal']['tpsummary'] = [
		'articles' => $max_art,
		'uploads' => $max_upload,
	];
}
...
function tp_profile_articles($member_id)
{
	...
	if ($context['TPortal']['tpsort'] != '') {
		...
		if (!in_array($sorting, ['date', 'subject', 'views', 'category', 'comments'])) {
			$sorting = 'date';
		}
	}
	...
	if ($smcFunc['db_num_rows']($request) > 0) {
		while ($row = $smcFunc['db_fetch_assoc']($request)) {
			...
			if ($can_see) {
				$context['TPortal']['profile_articles'][] = [
					'id' => $row['id'],
					'subject' => $row['subject'],
					'date' => timeformat($row['date']),
					'timestamp' => $row['date'],
...
function tp_profile_download($memID)
{
	...
	$context['TPortal']['profile_uploads'] = [];
	if (!in_array($sorting, ['name', 'created', 'views', 'downloads', 'category'])) {
		$sorting = 'created';
	}
	$request = $smcFunc['db_query'](
		'',
	...
	if ($value !== false) {
		...
		elseif ($member_id != false) {
			$smcFunc['db_insert'](
				'INSERT',
				'{db_prefix}tp_data',
				['type' => 'int', 'id_member' => 'int', 'value' => 'int'],
				[2, $member_id, $value],
				['id']
			);
		}
