...
function TPBlockActions(&$subActions)
{
	$subActions = array_merge(
		[
			'showblock' => ['TPBlock.php', 'showBlock',   []],
		],
		$subActions
	);
}
	...
	if (is_array($availableBlocks) && count($availableBlocks)) {
		foreach ($availableBlocks as $row) {
			...
			elseif ($row['type'] == TP_BLOCK_SHOUTBOX) {
				call_integration_hook('integrate_tp_shoutbox', [&$row]);
			}
...
function TPBlockAdminActions(&$subActions)
{
	$subActions = array_merge(
		[
			'editblock' => ['TPBlock.php', 'editBlock',   []],
			'deleteblock' => ['TPBlock.php', 'deleteBlock', []],
			'saveblock' => ['TPBlock.php', 'saveBlock',   []],
		],
		$subActions
	);
}
...
function adminBlocks()
{
	...
	if (isset($_GET['addpos']) || isset($_GET['subpos'])) {
		...
		if (isset($_GET['addpos'])) {
			...
			$current = $tpBlock->getBlockData(['pos', 'bar'], ['id' => $id]);
			$new = $current[0]['pos'] + 1;
			$existing = $tpBlock->getBlockData('id', ['bar' => $current[0]['bar'], 'pos' => $new]);
			if (is_array($existing)) {
				$tpBlock->updateBlock($existing[0]['id'], ['pos' => $current[0]['pos']]);
			}
		}
		else {
		...
		$tpBlock->updateBlock($id, ['pos' => $new]);
	...
	if (isset($_GET['blockon'])) {
		...
		$current = $tpBlock->getBlockData(['off'], ['id' => $id]);
		if (is_array($current)) {
			if ($current[0]['off'] == 1) {
				$tpBlock->updateBlock($id, ['off' => '0']);
			}
			else {
				$tpBlock->updateBlock($id, ['off' => '1']);
			}
		}
		redirectexit('action=tpadmin;sa=blocks');
	}
	...
	foreach (['blockright', 'blockleft', 'blockcenter', 'blockfront', 'blockbottom', 'blocktop', 'blocklower'] as $block_location) {
		if (array_key_exists($block_location, $_GET)) {
			...
			$tpBlock->updateBlock($id, ['bar' => $loc]);
			redirectexit('action=tpadmin;sa=blocks');
		}
	}
		...
		if (is_countable($blocks) && count($blocks) > 0) {
			...
			if (array_multisort($bar, SORT_ASC, $pos, SORT_ASC, $blocks)) {
				foreach ($blocks as $row) {
					// decode the block settings
					$set = json_decode($row['settings'], true) ?? [];
					$context['TPortal']['admin_' . $bars[$row['bar']] . 'block']['blocks'][] = $set + [
						'frame' => $row['frame'],
						'title' => $row['title'],
						'type' => $tpBlock->getBlockType($row['type']),
						'body' => $row['body'],
...
function editBlock($block_id = 0)
{
	...
	if (is_array($row)) {
		...
		$context['TPortal']['blockedit']['display'] = [
			'action' => [],
			'board' => [],
			'page' => [],
			'cat' => [],
		...
		if ($context['TPortal']['blockedit']['lang'] != '') {
			$context['TPortal']['blockedit']['langfiles'] = [];
		...
		get_articles();
		tp_getDLcats();
		$context['TPortal']['edit_categories'] = [];
