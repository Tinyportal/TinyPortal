...
function TPortal()
{
	...
	$subActions = [
		'credits' => ['TPhelp.php', 'TPCredits', []],
		'updatelog' => ['TPSubs.php', 'TPUpdateLog', []],
		'savesettings' => ['TPSubs.php', 'TPSaveSettings', []],
	];

	call_integration_hook('integrate_tp_pre_subactions', [&$subActions]);
...
function TPortal_init()
{
	...
	if (!isset($modSettings['jquery_source'])) {
		loadJavaScriptFile('https://ajax.googleapis.com/ajax/libs/jquery/3.3.1/jquery.min.js', ['external' => true], 'tp_jquery');
	}
	...
	elseif (!isset($_GET['action']) && !isset($_GET['board']) && !isset($_GET['topic'])) {
		doTPfrontpage();
	}
...
function tpLoadCSS()
{
	...
	if (!empty($settings['default_theme_url']) && !empty($settings['theme_url']) && file_exists($settings['theme_dir'] . '/css/tp-colors.css')) {
		$context['html_headers'] .= '<link rel="stylesheet" type="text/css" href="' . $settings['theme_url'] . '/css/tp-colors.css?' . TPVERSION . '" />';
	}
	else {
		$context['html_headers'] .= '<link rel="stylesheet" type="text/css" href="' . $settings['default_theme_url'] . '/css/tp-colors.css?' . TPVERSION . '" />';
	}
...
function setupTPsettings()
{
	...
	if (($context['TPortal'] = cache_get_data('tpSettings', 90)) == null) {
		...
		$request = $smcFunc['db_query'](
			'',
			'
                SELECT name, value FROM {db_prefix}tp_settings',
			[]
		);
...
function fetchTPhooks()
{
	...
	$types = ['art_not_approved', 'dl_not_approved'];
	...
	$context['TPortal']['submitcheck'] = ['articles' => 0, 'uploads' => 0];
	...
	if (!empty($_GET['page'])) {
		...
		if (is_array($article) && !empty($article)) {
			...
			if (get_perm($article['value3']) && $valid) {
				...
				$context['TPortal']['article']['avatar'] = set_avatar_data(
					[
				...
				$all = ['showtop', 'centerpanel', 'leftpanel', 'rightpanel', 'toppanel', 'bottompanel', 'lowerpanel'];
				$all2 = ['top', 'cblock', 'lblock', 'rblock', 'tblock', 'bblock', 'lbblock', 'comments', 'views', 'rating', 'date', 'title',
				...
				if (in_array('inherit', $context['TPortal']['article']['visual_options'])) {
					$all = ['leftpanel', 'rightpanel', 'toppanel', 'centerpanel', 'lowerpanel', 'bottompanel'];
					for ($p = 0; $p < 6; $p++) {
				...
				if (in_array('category', $context['TPortal']['article']['visual_options'])) {
					$request = $smcFunc['db_query'](
						...
						AND approved = 1
						ORDER BY parse',
						['cat' => $context['TPortal']['article']['category']]
					);
					if ($smcFunc['db_num_rows']($request) > 0) {
						$context['TPortal']['article']['others'] = [];
...
function doTPcat()
{
	...
	if (!empty($_GET['cat'])) {
		...
		$request = $smcFunc['db_query'](
			...
			SELECT * FROM {db_prefix}tp_variables
			WHERE ' . $catid . ' LIMIT 1',
			['cat' => is_numeric($cat) ? (int) $cat : $cat]
		);
		if ($smcFunc['db_num_rows']($request) > 0) {
			...
			if (get_perm($category['value3'])) {
				...
				$context['TPortal']['category']['articles'] = [];
				...
				$all = ['centerpanel', 'leftpanel', 'rightpanel', 'toppanel', 'bottompanel', 'lowerpanel'];
				for ($p = 0; $p < 6; $p++) {
				...
				// any children then?
				$allcats = [];
				$context['TPortal']['category']['children'] = [];
				$request = $smcFunc['db_query'](
					'',
					'
...
function doTPfrontpage()
{
	...
	// first, the panels
	foreach (['left', 'right', 'center', 'top', 'bottom', 'lower'] as $pan => $panel) {
	...
	if (!in_array($catsort, ['date', 'author_id', 'id', 'parse'])) {
		$catsort = 'date';
	}
	...
	if (!empty($context['TPortal']['uselangoption'])) {
		$display = TPUtil::find_in_set(['tlang=' . $user_info['language']], 'display');
		if (isset($display)) {
			$display = ' AND ' . $display;
		}
	}
	...
	if ($smcFunc['db_num_rows']($request) > 0) {
		while ($row = $smcFunc['db_fetch_assoc']($request)) {
			...
			elseif ($row['type'] == 20) {
				call_integration_hook('integrate_tp_blocks', [&$row]);
			}
			$can_manage = allowedTo('tp_blocks');

			$blocks[$panels[$row['bar']]][$count[$panels[$row['bar']]]] = $set + [
				'frame' => $row['frame'],
	...
	if (isset($test_articlebox) && $fetchart != '') {
		...
		if ($smcFunc['db_num_rows']($request) > 0) {
			while ($article = $smcFunc['db_fetch_assoc']($request)) {
				...
				$context['TPortal']['blockarticles'][$article['id']]['avatar'] = set_avatar_data(
					[
				...
				// sort out the options
				$context['TPortal']['blockarticles'][$article['id']]['visual_options'] = [];
	...
	if (isset($test_catbox) && $fetchtitles != '') {
		...
		if (!isset($context['TPortal']['blockarticle_titles'])) {
			$context['TPortal']['blockarticle_titles'] = [];
		}

		if ($smcFunc['db_num_rows']($request) > 0) {
			while ($row = $smcFunc['db_fetch_assoc']($request)) {
				$context['TPortal']['blockarticle_titles'][$row['category']][$row['date'] . '_' . $row['id']] = [
					'id' => $row['id'],
					'subject' => $row['subject'],
		...
		if ($smcFunc['db_num_rows']($request) > 0) {
			while ($row = $smcFunc['db_fetch_assoc']($request)) {
				...
				if ($row['value5'] != -1 && $row['value2'] != '-1') {
					...
					$context['TPortal']['menu'][$row['subtype2']][] = [
						'id' => $row['id'],
						'menuID' => $row['subtype2'],
						'name' => $row['value1'],
						'pos' => $menupos,
...
function TPortal_panel($side)
{
	...
	if (!isset($context['TPortal'][$panelside][$side])) {
		$context['TPortal'][$panelside][$side] = [];
	}
	...
	foreach ($context['TPortal'][$panelside][$side] as $i => &$block) {
		...
		if (in_array($flow, ['horiz', 'horiz2', 'horiz3', 'horiz4', 'grid'])) {
...
function tpSetupUpshrinks()
{
	...
	$panels = ['Left', 'Right', 'Top', 'Center', 'Lower', 'Bottom'];
	...
	$context['TPortal']['upshrinkblocks'] = [];
