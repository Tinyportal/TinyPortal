...
// Shoutbox Block template
function template_tpshout_shoutblock( $shoutbox_id = 0, $shoutbox_layout = null, $useavatar = null )
{
	    ...
	    if ( in_array($context['TPortal']['shoutbox_layout'], array('2','3'), true ) ) {
            ...
            echo '
                ...
                <div style="overflow: hidden;">
                   <a id="tp_shout_refresh_' . $shoutbox_id . '" onclick="TPupdateShouts(\'fetch\', '.$shoutbox_id.' , null , '.$shoutbox_layout.', '.$useavatar.' ); return false;" href="' , $scripturl , '?action=tpshout;shout=refresh" title="'. $txt['tp-shout-refresh'] . '"><img class="floatleft" src="' . $settings['tp_images_url'] . '/TPrefresh.png" alt="" /></a>
                    <input onclick="TPupdateShouts(\'save\', '.$shoutbox_id.' , null , '.$shoutbox_layout.', '.$useavatar.' ); return false;" type="submit" name="shout_send" value="&nbsp;'.$txt['shout!'].'&nbsp;" tabindex="', $context['tabindex']++, '" class="button_submit" />';
				if (allowedTo('tp_can_admin_shout')) {
					echo '
						<a href="' , $scripturl , '?action=tpshout;shout=admin;settings" title="'. $txt['tp-shoutboxsettings'] .'"><img class="floatright" src="' . $settings[	'tp_images_url'] . '/TPsettings.png" alt="" /></a>';
				}
...
// Shoutbox single shout template
function template_singleshout($row, $shoutbox_id, $shoutbox_layout = null, $useavatar = null)
{
	...
	<div style="padding-bottom: 5px;">
		<div class="tp_shoutbody_layout0">
			<div class="tp_shoutavatar">
				' . ($context['TPortal']['useavatar']=='1' ? '<div class="tp_shoutavatar2"><a href="' . $scripturl. '?action=profile;u=' . $row['member_id'] . '">' . $row['avatar'] . '</a></div>' : '') . '
				...
				<div class="tp_shoutbox_edit">
					...
					<a onclick="TPupdateShouts(\'del\', '.$shoutbox_id.', '.$row['id'].' , '.$shoutbox_layout.', '.$useavatar.' ); return false;" class="shout_delete" title="'.$txt['tp-delete'].'" href="' . $scripturl. '?action=tpshout;shout=del;s=' . $row['id'] . ';' . $context['session_var'] . '=' . $context['session_id'].'"><img src="' . $settings['tp_images_url'] . '/TPdelete_shout.png" alt="'.$txt['tp-delete'].'" /></a>
				</div>' : '') . '
	...
	if(!empty($layoutOptions[$suseavatar])) { 
		return $layoutOptions[$useavatar];
	}
}
