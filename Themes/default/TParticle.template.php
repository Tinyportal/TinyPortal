...
function template_submitarticle()
{
	...
	$tpmonths = [' ', 'Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'];
	if (!empty($context['TPortal']['editarticle'])) {
		$mg = $context['TPortal']['editarticle'];
	}
	else {
		// Set some defaults
		$mg = ['off' => 1, 'id' => '', 'body' => '', 'subject' => '', 'shortname' => '', 'useintro' => 0, 'date' => time(), 'intro' => ''];
	}
	...
	if (allowedTo('admin_forum') || allowedTo('tp_articles')) {
		...
		echo '
				...
				<input type="submit" class="button" name="' . $txt['tp-send'] . '" value="' . $txt['tp-send'] . '" >';
		$opts = ['', 'date', 'title', 'author', 'linktree', 'top', 'cblock', 'rblock', 'lblock', 'bblock', 'tblock', 'lbblock', 'category', 'catlist', 'comments', 'commentallow', 'commentupshrink', 'views', 'rating', 'ratingallow', 'nolayer', 'avatar', 'inherit', 'social', 'nofrontsetting'];
		...
		echo '
					...
					<dl class="tp_title settings">';
		$articleOption = [4, 2, 13, 3, 1, 17, 19, 18, 21, 23, 12, 15, 14];
