...
function do_articles()
{
	...
	if(isset($show_submission) && $context['TPortal']['total_submissions']>0)
	{
		...
		$request = tp_query("
			SELECT	art.id,art.date,art.frontpage, art.category, art.author_id as authorID, IFNULL(mem.real_name, art.author) as author, art.subject,
	...
	if(isset($whatarticle))
	{
		...
		$request = tp_query("
			SELECT	art.*, IFNULL(mem.real_name, art.author) as realName, art.author_id as authorID, art.type as articletype, art.id_theme as ID_THEME 
			FROM " . $tp_prefix . "articles as art
			LEFT JOIN " . $db_prefix . "members as mem ON (art.author_id = mem.id_member)
...
function do_postchecks()
{
	...
	if(!empty($_POST['tpadmin_form']))
	{
		...
		elseif($from == 'addblock')
		{
			...
			VALUES(" . $cp['type'] . ", '" . $cp['frame'] . "', '" . mysql_real_escape_string($title) . "', '" . htmlentities($cp['body'],ENT_QUOTES) . "','" . $cp['access'] . "', 
			" . $panel .", 0, 1,1," . $cp['var1'] . "," . $cp['var2'] . ",'" . $cp['lang'] . "','" . $cp['access2'] . "','" . $cp['editgroups'] . "') " , __FILE__, __LINE__);
			else
				$request = tp_query("INSERT INTO " . $tp_prefix . "blocks 
			(type,frame,title,body,access,bar,pos,off,visible,var1,var2,lang,access2,editgroups)
			VALUES(" . $type . ", 'theme', '" . mysql_real_escape_string($title) . "', '" . htmlentities($body, ENT_QUOTES) . "', '-1,0,1', " . $panel .", 0, 1,1,0,0,'','actio=allpages','') " , __FILE__, __LINE__);
		...
		elseif($from == 'blockedit')
		{
			...
			if(isset($_POST['blockcode_overwrite']))
			{
				...
				mysql_query("UPDATE " . $tp_prefix . "blocks SET body='" . addslashes($newval['code']) . "' WHERE id=" . $where);
			}
		...
		elseif(substr($from,0,11) == 'editarticle')
		{
			...
			foreach($_POST as $what => $value)
			{
				if(substr($what, 0,11) == 'tp_article_' && !empty($where))
				{
					...
					if($setting=='authorid')
					{
						tp_query("UPDATE " . $tp_prefix . "articles SET author_id = " . $value . " WHERE id=" . $where  . " LIMIT 1", __FILE__, __LINE__);
					}
					...
					{
						tp_query("UPDATE " . $tp_prefix . "articles SET subject = '" . mysql_real_escape_string($value) . "' WHERE id='" . $where  . "' LIMIT 1", __FILE__, __LINE__);
					}
					...
					elseif(in_array($setting, array('body','intro')))
					{
						...
						mysql_query("UPDATE " . $tp_prefix . "articles SET " . $setting . " = '" . mysql_real_escape_string($value) . "' WHERE id='" . $where  . "' LIMIT 1");
					}
					...
					elseif(in_array($setting, array('pubstartday','pubstartmonth','pubstartyear','pubstartminute','pubstarthour','pub_start')))
					{
						...
						if(empty($_POST['tp_article_pubstarthour']) && empty($_POST['tp_article_pubstartminute']) && empty($_POST['tp_article_pubstartmonth']) && empty($_POST['tp_article_pubstartday']) && empty($_POST['tp_article_pubstartyear']))
								tp_query("UPDATE " . $tp_prefix . "articles SET pub_start = 0 WHERE id='" . $where  . "' LIMIT 1", __FILE__, __LINE__);
						else
							...
							if(!isset($pubstart))
								tp_query("UPDATE " . $tp_prefix . "articles SET pub_start = " . $timestamp . " WHERE id='" . $where  . "' LIMIT 1", __FILE__, __LINE__);
					...
					elseif(in_array($setting, array('pubendday','pubendmonth','pubendyear','pubendminute','pubendhour','pub_start')))
					{
						...
						if(empty($_POST['tp_article_pubendhour']) && empty($_POST['tp_article_pubendminute']) && empty($_POST['tp_article_pubendmonth']) && empty($_POST['tp_article_pubendday']) && empty($_POST['tp_article_pubendyear']))
								tp_query("UPDATE " . $tp_prefix . "articles SET pub_end = 0 WHERE id='" . $where  . "' LIMIT 1", __FILE__, __LINE__);
						else
							...
							if(!isset($pubend))
								tp_query("UPDATE " . $tp_prefix . "articles SET pub_end = " . $timestamp . " WHERE id='" . $where  . "' LIMIT 1", __FILE__, __LINE__);
			...
			if(isset($_POST['tparticle_itemtags']))
			{
				...
				foreach($_POST as $what => $value)
				{
					...
					if(substr($what,0,18)=='tparticle_itemtags' && !empty($value))
					{
						...
						// insert new one
						$href='?page='.$itemid;
						$subject = '<div  class="taglink">' . html_entity_decode($title[0]). '</div>';
						if(!empty($tag))
						{
							tp_query("INSERT INTO " . $tp_prefix . "variables (value1,value2,value3,type,value4,value5,subtype,value7,value8,subtype2) 
							VALUES('" . $href. "','". mysql_real_escape_string($subject) ."','tparticle_itemtags','globaltag_item','',0,'" . $tag. "','',''," . $itemid . ")", __FILE__, __LINE__);
							$mytags[] = $tag;
						}
					}
					elseif(substr($what,0,24)=='xyzx_tparticle_itemtags_' && !empty($value))
					{
						...
						// insert new one
						$href='?page='.$itemid;
						$subject = '<span style="background: url('.$settings['tp_images_url'].'/glyph_article.png) no-repeat;" class="taglink">' . html_entity_decode($title[0]). '</span>';
						if(!empty($tag))
						{
							tp_query("INSERT INTO " . $tp_prefix . "variables (value1,value2,value3,type,value4,value5,subtype,value7,value8,subtype2) 
							VALUES('$href','". mysql_real_escape_string($subject) . "','tparticle_itemtags','globaltag_item','',0,'$tag','','',$itemid)", __FILE__, __LINE__);
