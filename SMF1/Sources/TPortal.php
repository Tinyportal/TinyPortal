...
{
	global $maintenance, $db_prefix, $context, $scripturl,$txt , $user_info, $settings , $modSettings, $ID_MEMBER, $boarddir, $boardurl, $sourcedir, $forum_version;
	...
	// load both stylesheets to be sure all is in, but not if thinsg aren't setup!
	...
	if((isset($_GET['action']) && $_GET['action']=='permissions') || (isset($_GET['sa']) && $_GET['sa']=='permissions'))
...
function TP_loadTheme()
{
    ...
    if(isset($_GET['page']) && !isset($_GET['action']))
	{
		...
		if(is_numeric($pag))
			$request =  tp_query("SELECT ID_THEME FROM " . $tp_prefix . "articles WHERE id=" .$pag, __FILE__, __LINE__);
		else
			$request =  tp_query("SELECT ID_THEME FROM " . $tp_prefix . "articles WHERE shortname='" .$pag. "'", __FILE__, __LINE__);
	...
	{
		// fetch the custom theme if any
		$request =  tp_query("SELECT COUNT(*) FROM " . $tp_prefix . "settings WHERE name='front_type' AND value='single_page' ", __FILE__, __LINE__);
		if(tpdb_num_rows($request)>0)
		{
			$row=tpdb_fetch_row($request);
			tpdb_free_result($request);
			$request =  tp_query("SELECT art.ID_THEME, sett.value FROM (" . $tp_prefix . "articles AS art, " . $tp_prefix . "settings AS sett) WHERE sett.name='featured_article' AND sett.value=art.id ", __FILE__, __LINE__);
			if(tpdb_num_rows($request)>0)
			{
				$row=tpdb_fetch_row($request);
				$theme=$row[0];
	...
	{
		// fetch the custom theme if any
		$request =  tp_query("SELECT value FROM " . $tp_prefix . "settings WHERE name='dlmanager_theme'", __FILE__, __LINE__);
		if(tpdb_num_rows($request)>0)
		{
			$row=tpdb_fetch_row($request);
			$theme=$row[0];
...
function setupTPsettings()
{
	...
	// get the settings
	$request =  tp_query("SELECT name,value FROM " . $tp_prefix . "settings", __FILE__, __LINE__);
	if (tpdb_num_rows($request) > 0)
	{
...
{
	global $maintenance, $tp_prefix, $db_prefix, $context, $scripturl,$txt , $user_info, $settings , $modSettings, $ID_MEMBER, $boarddir, $boardurl, $sourcedir, $forum_version;
	...
	if($what_board>0)
	{
		$request2 =  tp_query("SELECT * FROM " . $tp_prefix . "variables WHERE type='layerhook' AND	value1='what_board'", __FILE__, __LINE__);
		$param=$what_board;
	}
	// perhaps a topic then?
	elseif($what_topic>0)
	{
		$request2 =  tp_query("SELECT * FROM " . $tp_prefix . "variables WHERE type='layerhook' AND	value1='what_topic'", __FILE__, __LINE__);
		$param=$what_topic;
	}
	// alright, an article?
	elseif($what_page>0)
	{
		$request2 =  tp_query("SELECT * FROM " . $tp_prefix . "variables WHERE type='layerhook' AND	value1='what_page'", __FILE__, __LINE__);
		$param=$what_page;
	}
	// a category of articles?
	elseif($what_cat>0)
	{
		$request2 =  tp_query("SELECT * FROM " . $tp_prefix . "variables WHERE type='layerhook' AND	value1='what_cat'", __FILE__, __LINE__);
		$param=$what_cat;
	}
	// guess neither..
	else
	...
	// do the actual hooks
	if(!empty($request2) && tpdb_num_rows($request2)>0)
	{
	...
	$request2 =  tp_query("SELECT * FROM " . $tp_prefix . "variables WHERE type IN ('" . implode("','" ,$types) . "')", __FILE__, __LINE__);
...
function doTPpage()
{
	...
	if(!empty($_GET['page']))
	{
		...
		if(!empty($page))
		{
			$request =  tp_query("SELECT art.*, art.authorID, art.ID_THEME, var.value1,var.value2,var.value3,var.value4, var.value5,var.value7,var.value8, art.type as rendertype, 
				IFNULL(mem.realName,art.author) as realName, mem.avatar, mem.posts, mem.dateRegistered,mem.lastLogin,
				IFNULL(a.ID_ATTACH, 0) AS ID_ATTACH, a.filename, a.attachmentType, var.value9 
				FROM " . $tp_prefix . "articles as art 
				LEFT JOIN " . $db_prefix . "members AS mem ON (mem.ID_MEMBER = art.authorID)
				LEFT JOIN " . $db_prefix . "attachments AS a ON (a.ID_MEMBER = art.authorID)
				LEFT JOIN " . $tp_prefix . "variables as var ON (var.id= art.category) 
				WHERE " . (is_numeric($page) ? "art.id=".$page : "art.shortname='".$page."'") . " 
				LIMIT 1", __FILE__, __LINE__);
			if(tpdb_num_rows($request)>0)
			{
				$article = tpdb_fetch_assoc($request);
				$valid=true;
				tpdb_free_result($request);
				...
				{	
					...
					$now = time();
					$request = tp_query("SELECT item FROM " . $tp_prefix . "data WHERE ID_MEMBER=" . $ID_MEMBER . " AND type=1 AND value=" . $article['id'] . " LIMIT 1", __FILE__, __LINE__);
					if(tpdb_num_rows($request)>0)
					{
						$row=tpdb_fetch_row($request);
						$last=$row[0];
						tpdb_free_result($request);
						$request =  tp_query("UPDATE " . $tp_prefix . "data SET item=" . $now . " WHERE ID_MEMBER=" . $ID_MEMBER . " AND type=1 AND value=" . $article['id'], __FILE__, __LINE__);
					}
					else
					{
						$last = $now;
						$request = tp_query("INSERT INTO " . $tp_prefix . "data (type, ID_MEMBER, value, item) VALUES (1," . $ID_MEMBER . ", " . $article['id'] . ", '" . $now. "')", __FILE__, __LINE__);
					}

					require_once($sourcedir . '/TPcommon.php');

					// ok, how manya rticles have this member posted?
					$request =  tp_query("SELECT id FROM " . $tp_prefix . "articles 
					WHERE authorID = " . $context['TPortal']['article']['authorID'] . "
					AND off= 0", __FILE__, __LINE__);
					if(tpdb_num_rows($request)>0)
					{
						$context['TPortal']['article']['countarticles'] = tpdb_num_rows($request);
						tpdb_free_result($request);
					}
					else
					...
					// fetch any comments
					$request =  tp_query("SELECT var.* , IFNULL(mem.realName,0) as realName,mem.avatar,
					...
					AND var.value5= " . $article['id'] . "
					ORDER BY var.value4 ASC", __FILE__, __LINE__);
					...
					if($ccount!=$article['comments'])
						tp_query("UPDATE " . $tp_prefix . "articles SET comments= " . $ccount . " WHERE id=" . $article['id'], __FILE__, __LINE__);
					...
					else
					{
						...
						$allcats = array();
						$request =  tp_query("SELECT * FROM " . $tp_prefix . "variables WHERE type = 'category'", __FILE__, __LINE__);
						if(tpdb_num_rows($request)>0)
						{
							while($row = tpdb_fetch_assoc($request))
								$allcats[$row['id']] = $row;
...
function doTPcat()
{
	...
	if(!empty($_GET['cat']))
	{
		...
		if(!empty($cat))
		{
			...
			$request =  tp_query("SELECT * FROM " . $tp_prefix . "variables WHERE " . (is_numeric($cat) ? "id=" . $cat : "value8 = '".$cat."'") . "	LIMIT 1", __FILE__, __LINE__);
			if(tpdb_num_rows($request)>0)
			{
				$category=tpdb_fetch_assoc($request);
				tpdb_free_result($request);
				// check permission
				if(get_perm($category['value3']))
				{
					...
					$request =  tp_query("SELECT art.id, IF(art.useintro>0, art.intro, art.body) AS body,
						art.date, art.category, art.subject, art.authorID, art.frame, art.comments, art.options,
						...
						LEFT JOIN " . $db_prefix . "attachments AS a ON (a.ID_MEMBER = mem.ID_MEMBER)
						WHERE art.category=" . $category['id'] . " 
						AND art.off=0 
						AND art.approved=1 
					...
					if(tpdb_num_rows($request)>0)
					{
						$total = tpdb_num_rows($request); $col1 = ceil($total/2);
					...
					$request =  tp_query("SELECT cat.*, COUNT(art.id) as articlecount
						FROM (" . $tp_prefix . "variables as cat)
						LEFT JOIN " . $tp_prefix . "articles as art ON (art.category = cat.id)
						WHERE cat.type = 'category' GROUP BY art.category", __FILE__, __LINE__);
					if(tpdb_num_rows($request)>0)
					{
						while($row = tpdb_fetch_assoc($request))
						{
					...
					$request =  tp_query("SELECT COUNT(*) FROM " . $tp_prefix . "articles WHERE category=" . $category['id'] . " AND off=0 AND approved=1", __FILE__, __LINE__);
					if(tpdb_num_rows($request)>0)
					{
						$row=tpdb_fetch_row($request);
						$all_articles=$row[0];
...
function doTPfrontpage()
{
	...
	if(!in_array($catsort, array('date','authorID','id','parse')))
		$catsort = 'date';
	...
	if($context['TPortal']['front_type'] == 'articles_only')
	{
		...
		$request =  tp_query("SELECT art.id
			FROM (" . $tp_prefix . "articles AS art, " . $tp_prefix . "variables AS var) 
			WHERE art.off = 0 
			AND var.id = art.category
			" . $artgroups . "
			AND art.category>0
			AND art.approved = 1 
			AND art.frontpage = 1", __FILE__, __LINE__);

		if(tpdb_num_rows($request)>0)
		{
			$articles_total = tpdb_num_rows($request);
			tpdb_free_result($request);
		}	
		else
			$articles_total = 0;;
		...
		$request =  tp_query("SELECT art.id, IF(art.useintro>0, art.intro, art.body) AS body,
			art.date, art.category, art.subject, art.authorID, var.value1 as category_name,
	...
	elseif(in_array($context['TPortal']['front_type'], array('forum_only','forum_selected')))
	{
		...
		$request =  tp_query("
			...
			AND t.ID_BOARD IN(" . $context['TPortal']['SSI_board'] .")
			" . ($context['TPortal']['allow_guestnews']==0 ? 'AND ' . $user_info['query_see_board'] : '') . "
			ORDER BY t.ID_FIRST_MSG DESC
			LIMIT " . $totalmax, __FILE__, __LINE__);
		else
			$request =  tp_query("
			SELECT t.ID_FIRST_MSG
			FROM (" . $db_prefix . "topics as t, " . $db_prefix . "boards as b)
		...
		// do some conversion
		if($catsort == 'date') { $catsort = 'posterTime'; } 
		...
		$request =  tp_query("
			SELECT 
			m.subject,  " . ($context['TPortal']['frontpage_limit_len']>0 ? "LEFT(m.body," . $context['TPortal']['frontpage_limit_len'] . ") as body" : "m.body") . ", 
			IFNULL(mem.realName, m.posterName) AS realName, m.posterTime as date, mem.avatar,mem.posts, mem.dateRegistered,mem.lastLogin,
			IFNULL(a.ID_ATTACH, 0) AS ID_ATTACH, a.filename, a.attachmentType, t.ID_BOARD as category, b.name as category_name,
			t.numReplies, t.ID_TOPIC as id, m.ID_MEMBER as authorID, t.numViews as views,t.numReplies as replies, t.locked,
			IFNULL(thumb.ID_ATTACH, 0) AS thumb_id, thumb.filename as thumb_filename
			FROM (" . $db_prefix . "topics AS t, " . $db_prefix . "messages AS m)
			LEFT JOIN " . $db_prefix . "members AS mem ON (mem.ID_MEMBER = m.ID_MEMBER)
			LEFT JOIN " . $db_prefix . "attachments AS a ON (a.ID_MEMBER = m.ID_MEMBER)
			LEFT JOIN " . $db_prefix . "attachments AS thumb ON (t.ID_FIRST_MSG = thumb.ID_MSG AND thumb.attachmentType = 3)
			LEFT JOIN " . $db_prefix . "boards AS b ON (b.ID_BOARD = t.ID_BOARD)
			WHERE t.ID_FIRST_MSG IN (" . implode(', ', $posts) . ")
			AND m.ID_MSG = t.ID_FIRST_MSG
			ORDER BY m." . $catsort . " DESC
			LIMIT " . $start . "," . $max, __FILE__, __LINE__);
	...
	elseif(in_array($context['TPortal']['front_type'], array('forum_articles', 'forum_selected_articles')))
	{
		...
		$request =  tp_query("SELECT art.id, art.date, art.sticky, art.featured
			FROM (" . $tp_prefix . "articles AS art, " . $tp_prefix . "variables AS var) 
			WHERE art.off = 0 
			AND var.id = art.category
			" . $artgroups . "
			...
			AND art.approved = 1 
			AND (art.frontpage = 1 OR art.featured = 1)
			ORDER BY art.featured DESC, art.sticky desc, art.date desc", __FILE__, __LINE__);
		...
		if($context['TPortal']['front_type'] == 'forum_articles')
			$request =  tp_query("
			SELECT t.ID_FIRST_MSG, m.posterTime as date
		...
		{
			// Find the thumbs.
			$request =  tp_query("SELECT ID_THUMB FROM " . $db_prefix . "attachments
				WHERE ID_MSG IN (" . implode(',', $mposts) . ") and ID_THUMB > 0" , __FILE__, __LINE__);
		...
		if(count($mposts)>0)
			$request =  tp_query("
			SELECT 
			m.subject, " . ($context['TPortal']['frontpage_limit_len']>0 ? "LEFT(m.body," . $context['TPortal']['frontpage_limit_len'] . ") as body" : "m.body") . ", IFNULL(mem.realName, m.posterName) AS realName, m.posterTime as date, mem.avatar,mem.posts, mem.dateRegistered,mem.lastLogin,
			IFNULL(a.ID_ATTACH, 0) AS ID_ATTACH, a.filename, a.attachmentType, t.ID_BOARD as category, b.name as category_name,
			t.numReplies, t.ID_TOPIC as id, m.ID_MEMBER as authorID, t.numViews as views,t.numReplies as replies, t.locked,
			IFNULL(thumb.ID_ATTACH, 0) AS thumb_id, thumb.filename as thumb_filename
			FROM (" . $db_prefix . "topics AS t, " . $db_prefix . "messages AS m)
			...
			LEFT JOIN " . $db_prefix . "attachments AS thumb ON (t.ID_FIRST_MSG = thumb.ID_MSG AND thumb.attachmentType = 3)
			LEFT JOIN " . $db_prefix . "boards AS b ON (b.ID_BOARD = t.ID_BOARD)
			WHERE t.ID_FIRST_MSG IN (" . implode(', ', $mposts) . ")
			AND m.ID_MSG = t.ID_FIRST_MSG
				ORDER BY date DESC, thumb.ID_ATTACH ASC", __FILE__, __LINE__);
		...
		if(count($aposts)>0)
		{
			...
			LEFT JOIN " . $db_prefix . "members AS mem ON (art.authorID = mem.ID_MEMBER)
			LEFT JOIN " . $db_prefix . "attachments AS a ON (a.ID_MEMBER = mem.ID_MEMBER)
			WHERE art.id IN (" . implode(', ', $aposts) . ")
			ORDER BY art.featured desc, art.sticky desc, art." . $catsort ." " . $catsort_order , __FILE__, __LINE__);
			if(tpdb_num_rows($request)>0)
			{
				while($row = tpdb_fetch_assoc($request))
				{
    ...
    if(isset($test_articlebox) && $fetchart!='')
	{
		...
		$request =  tp_query("SELECT art.*, var.value1,var.value2,var.value3,var.value4, var.value5,var.value7,var.value8, art.type as rendertype, 
		IFNULL(mem.realName,art.author) as realName, mem.avatar, mem.posts, mem.dateRegistered,mem.lastLogin,
		IFNULL(a.ID_ATTACH, 0) AS ID_ATTACH, a.filename, a.attachmentType, var.value9 
		FROM " . $tp_prefix . "articles as art 
		LEFT JOIN " . $db_prefix . "members AS mem ON (mem.ID_MEMBER = art.authorID)
		...
		AND art.category>0
		AND art.approved=1 
		AND art.category>0 AND art.category<9999	", __FILE__, __LINE__);
		if(tpdb_num_rows($request)>0)
		{
   ...
   // any cat listings from blocks?
	...
	if(isset($test_menubox))
	{
		...
		$request =  tp_query("SELECT * FROM " . $tp_prefix . "variables WHERE type='menubox' ORDER BY value5 ASC ", __FILE__, __LINE__);
		if(tpdb_num_rows($request)>0)
		{
			while ($row = tpdb_fetch_assoc($request))
			{
		...
		$request =  tp_query("SELECT * FROM " . $tp_prefix . "variables WHERE type='menutype' ORDER BY value5 ASC ", __FILE__, __LINE__);
		if(tpdb_num_rows($request)>0)
		{
			while ($row = tpdb_fetch_assoc($request))
			{
...
function doTPblocks()
{
	...
	$request =  tp_query("SELECT * FROM 
		...
		AND " . $access . "
		" . $lang . "
		ORDER BY bar,pos,id ASC", __FILE__, __LINE__);
...
function TPortal_panel($side)
{
	...
	$code = '
	<div class="tp_'.$side.'panel"' . ($flow == 'horiz' ? ' style="overflow: hidden;"' : '') . '>';
	// set the grid type
	if($flow == 'grid')
	{
...
{	
	...
	if($context['user']['is_logged'])
	{
		...
		$result =  tp_query("SELECT type,value, item FROM " . $tp_prefix . "data WHERE type=2 AND ID_MEMBER=" . $ID_MEMBER, __FILE__, __LINE__);
