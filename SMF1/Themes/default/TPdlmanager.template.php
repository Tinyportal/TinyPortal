<?php
// Version: TinyPortal 1.0; TPdlmanager
...
function template_main()
{
	...
	if($context['TPortal']['show_download']==0 && !allowedTo('tp_dlmanager'))
		...
		echo '
	<div class="tborder"><h3 class="titlebg"><span class="left"></span>', !empty($context['TPortal']['dlheader']) ? $context['TPortal']['dlheader'] : $txt['tp-downloads'], '	</h3></div>
	<div>';
		$dlbuttons = array(
		...
		echo '
	<table cellspacing="0" cellpadding="0" align="right" style="margin-right: 10px; margin-bottom: 8px;" class="middletext">';
		tp_template_button_strip($dlbuttons, 'top');
	...
	if($context['TPortal']['dlaction']=='' || $context['TPortal']['dlaction']=='cat')
	{
		...
		if(!empty($context['TPortal']['dl_introtext']) && $context['TPortal']['dlaction']=='')
			echo '
	...
	<div class="tborder">
		...
		<div style="padding: 1em;" class="windowbg2">';
	...
	<div class="tborder">
		<h3 class="catbg"><span class="l"></span><span class="r"></span>'.$txt['tp-categories'] .'</h3>
		<div class="windowbg">';
		...
		foreach($context['TPortal']['dlcats'] as $dlcat)
		{
			...
			echo '
			<div class="windowbg2 dlcategory"' , !empty($content) ? ' style="margin-bottom: 0;"' : '' ,'>
			...
			if(!empty($content))
				echo '
			<div class="dlcategory windowbg" style="padding-left: 5em;"><ul class="tp-subcategories">'.$content.'</ul></div>';
		}
		...
		<div class="windowbg2" style="padding: 1em;">
			...
			foreach($context['TPortal']['dlitem'] as $dlitem)
			{
				...
				<div class="dlitemgrid">
					...
					<div class="windowbg2" style="float: right; padding: 0 2em; margin: 0 0 1em 0;">' , $dlitem['icon']!='' ? '<img src="'. (substr($dlitem['icon'],0,4)=='http' ? $dlitem['icon'] :  $boardurl. '/' . $dlitem['icon']).'" border="0" alt="'.$dlitem['name'].'"  />' : '<img src="' . $settings['tp_images_url'] . '/TPnodl.png" alt="" />' , '	</div>';
	...
	<div class="tborder">
		...
		foreach($context['TPortal']['dlitem'] as $dlitem)
		{
			echo '
			<div class="tp_half">
				<h3 class="catbg"><span class="l"></span><span class="r"></span>
		...
		if(!empty($dlitem['sshot']))
			 echo '
				<br /><div style="width: '.$context['TPortal']['dl_screenshotsize'][2].'px; height: '.$context['TPortal']['dl_screenshotsize'][3].'px;background: url('.$dlitem['sshot'].') no-repeat;"></div>';
		echo '
			</div>
	...
	elseif($context['TPortal']['dlaction']=='upload')
	{
	   ...
	   echo '
	 <div class="tborder">
		<h4 class="titlebg"><span class="left"></span>'.$txt['tp-dlupload'].'</h4>
		<div class="windowbg">
	...
	if($context['TPortal']['dlaction']=='stats')
	{
	   ...
	   echo '
	<div class="tborder"">
		<h3 class="catbg"><span class="left"></span>'.$txt['tp-downloadsection'].' - '.$txt['tp-stats'].'</h3>
	...
	if($context['TPortal']['dlaction']=='useredit')
	{
		...
		<form accept-charset="', $context['character_set'], '" name="dl_useredit" action="'.$scripturl.'?action=tpmod;dl=admin" enctype="multipart/form-data" onsubmit="syncTextarea();" method="post">
			<table width="100%" cellspacing="1" cellpadding="7" class="tborder">
				...
				if($context['TPortal']['dl_wysiwyg'] == 'html')
					TPwysiwyg('dladmin_text'.$cat['id'], html_entity_decode($cat['description'], ENT_QUOTES, $context['characer_set']), true,'qup_dladmin_text');
				elseif($context['TPortal']['dl_wysiwyg'] == 'bbc')
					TP_bbcbox('dl_useredit','dladmin_text'.$cat['id'], html_entity_decode($cat['description'], ENT_QUOTES, $context['characer_set']));
				else
					echo '<textarea name="dladmin_text'.$cat['id'].'" style="width: 99%; height: 300px;">'. html_entity_decode($cat['description'], ENT_QUOTES, $context['characer_set']).'</textarea>';
	...
	if($context['TPortal']['dlaction']=='search')
	{
	   echo '
		<div class="tborder">
			<h3 class="catbg"><span class="left"></span>'.$txt['tp-downloadsection'].' - '.$txt['tp-dlsearch'].'</h3>
			<div class="windowbg">
				<span class="topslice"><span></span></span>
	...
	if($context['TPortal']['dlaction']=='results')
	{
		echo '
		<div class="tborder">
			<h3 class="catbg3" style="padding: 5px 10px; margin: 0;">' , $txt['tp-dlsearchresults'] , '
