...
function template_blockoverview()
{
	...
	<form accept-charset="', $context['character_set'], '" name="tpadmin_news" action="' . $scripturl . '?action=tpadmin" method="post" style="margin: 0px;">
		...
		<table class="admintable">
			<caption class="catbg">' . $txt['tp-blockoverview'] . '</caption>
			<tbody>
				<tr class="windowbg2">
					<td class="tborder" style="padding: 0; border: none;">
						<table class="formtable">';
		...
		if(allowedTo('tp_blocks') && isset($context['TPortal']['blockoverview']))
		{
			...
			if(!isset($_GET['grp']))
			{
				foreach($context['TPortal']['blockoverview'] as $block)
				{
					...
					<tr class="windowbg">
						<td colspan="2">
							<div style="padding: 1em 0; overflow: hidden;"> 
								<a href="' . $scripturl . '?action=tpadmin;blockedit='.$block['id'].';' . $context['session_var'] . '=' . $context['session_id'].'" title="'.$txt['tp-edit'].'"><b>' . $block['title'] . '</a></b> ( ' . $txt['tp-blocktype' . $block['type']] . ' | ' . $txt['tp-' .$side[$block['bar']]] . ')
							</div>
							<div class="dl_perm" id="tp'.$block['id'].'" style="overflow: hidden;">
								<input type="hidden" value="control" name="' . rand(10000,19999) .'tpblock'.$block['id'].'" />';

					foreach($context['TPmembergroups'] as $grp)
						echo '
								<div class="perm middletext">
									<input type="checkbox" id="tpb'.$block['id'] . '-' . $grp.'" value="' . $grp['id'].'" ' , in_array($grp['id'],$block['access']) ? 'checked="checked" ' : '' , ' name="' . rand(10000,19999) .'tpblock'.$block['id'].'" />'. $grp['name'].'
								</div>';
					...
					echo '
							...
							<hr style="clear: both;" /><input type="checkbox" onclick="invertAll(this, this.form, \'tpb'.$block['id'].'\');" />'.$txt['tp-checkall'],'
					</td></tr>';
				}
			}
		}
			...
			echo '
								<tr class="windowbg2">
									...
									<td class="smalltext" align="center">
										<a href="' . $scripturl . '?action=tpadmin;cu=-1;sesc='.$context['session_id'].';artdelete=' .$alink['id']. '" onclick="javascript:return confirm(\''.$txt['tp-articleconfirmdelete'].'\')">
				...
				foreach($context['TPortal']['menubox'][$mid] as $lbox){
					echo '
							<tr class="windowbg' , $lbox['off']=='0' ? '2' : '' , '">
								...
								<td><a href="' . $scripturl . '?action=tpadmin;linkedit=' .$lbox['id']. ';sesc='.$context['session_id'].'">' .$lbox['name']. '</a></td>
								<td>
					...
					echo '
								...
								<td align="left" nowrap="nowrap">&nbsp;
									<a href="' . $scripturl . '?action=tpadmin;linkon=' .$lbox['id']. ';mid=' , (isset($_GET['mid']) && is_numeric($_GET['mid'])) ? $_GET['mid'] : 0 , ';sesc='.$context['session_id'].'"><img title="'.$txt['tp-activate'].'" border="0" src="' .$settings['tp_images_url']. '/TPgreen' , $lbox['off']==1 ? '2' : '' , '.gif" alt="'.$txt['tp-activate'].'"  /></a>
									<a href="' . $scripturl . '?action=tpadmin;linkoff=' .$lbox['id']. ';mid=' , (isset($_GET['mid']) && is_numeric($_GET['mid'])) ? $_GET['mid'] : 0 , ';sesc='.$context['session_id'].'"><img title="'.$txt['tp-deactivate'].'" border="0" src="' .$settings['tp_images_url']. '/TPred' , $lbox['off']==0 ? '2' : '' , '.gif" alt="'.$txt['tp-deactivate'].'"  /></a>
									<a href="' . $scripturl . '?action=tpadmin;linkedit=' .$lbox['id']. ';mid=' , (isset($_GET['mid']) && is_numeric($_GET['mid'])) ? $_GET['mid'] : 0 , ';sesc='.$context['session_id'].'"><img title="'.$txt['tp-edit'].'" border="0" src="' .$settings['tp_images_url']. '/TPconfig_sm.gif" alt="'.$txt['tp-edit'].'"  /></a>
								</td>
								<td>';
					if($lbox['type']=='cats'){
						...
						foreach($context['TPortal']['editcats'] as $bmg)
						{
							if($lbox['IDtype']==$bmg['id']) 
								echo html_entity_decode($bmg['name'], ENT_QUOTES, $context['characer_set']);
						}
					}
					elseif($lbox['type']=='arti'){
						// or a arti (article)?
						...
						{
							if($lbox['IDtype']==$bmg['id'])
								echo html_entity_decode($bmg['subject'], ENT_QUOTES, $context['characer_set']);
						}
					}
					elseif($lbox['type']=='head'){
						// or a head (header)?
					...
					echo '
								...
								<td align="left" nowrap="nowrap">&nbsp;
									<a href="' . $scripturl . '?action=tpadmin;sesc='.$context['session_id'].';linkdelete=' .$lbox['id']. '" onclick="javascript:return confirm(\''.$txt['tp-suremenu'].'\')"><img title="'.$txt['tp-delete'].'" border="0" src="' .$settings['tp_images_url']. '/TPdelete2.gif" alt="'.$txt['tp-delete'].'"  /></a>
								</td>
							</tr>';
					$n++;
				}
					...
					echo '
						<tr class="windowbg2">
							...
							<td class="windowbg2" colspan="4" align="left" nowrap="nowrap">&nbsp;
								<a href="' . $scripturl . '?action=tpadmin;sesc='.$context['session_id'].';linkdelete=' .$mbox['id']. ';fullmenu" onclick="javascript:return confirm(\''.$txt['tp-suremenu'].'\')"><img title="'.$txt['tp-delete'].'" border="0" src="' .$settings['tp_images_url']. '/TPdelete2.gif" alt="'.$txt['tp-delete'].'"  /></a>
							</td>
						</tr>';
			}		
		}
	...
	<form accept-charset="', $context['character_set'], '" name="tpadminmenu" action="' . $scripturl . '?action=tpadmin" method="post" style="margin: 0px;">
		...
		<table class="admintable">
			...
			if(count($context['TPortal']['editcats'])>0){
				...
				foreach($context['TPortal']['editcats'] as $bmg){
					echo '
							<option value="', $bmg['id'] ,'">'. str_repeat("-",($bmg['indent'])) .' '. html_entity_decode($bmg['name'], ENT_QUOTES, $context['characer_set']).'</option>';
				}
			}
			//  (article)
			echo '
			...
			if(count($context['TPortal']['edit_articles'])>0){
				foreach($context['TPortal']['edit_articles'] as $bmg){
					echo '
									<option value="', empty($bmg['shortname']) ? $bmg['id'] : $bmg['shortname'] ,'"> '.html_entity_decode($bmg['subject'], ENT_QUOTES, $context['characer_set']).'</option>';
				}
			}
			else
				echo '
...
function template_linkmanager()
{
	...
	<form accept-charset="', $context['character_set'], '" name="tpadminmenu" action="' . $scripturl . '?action=tpadmin" method="post" style="margin: 0px;">
		...
		if(count($context['TPortal']['editcats'])>0){
			foreach($context['TPortal']['editcats'] as $bmg){
 				echo '
 								<option value="',  $bmg['id']  ,'"' , $context['TPortal']['editmenuitem']['type']=='cats' && $context['TPortal']['editmenuitem']['IDtype']==$bmg['id'] ? ' selected' : ''  ,' > '. html_entity_decode($bmg['name'], ENT_QUOTES, $context['characer_set']).'</option>';
			}
		}
		else
 			echo '
		...
		if(count($context['TPortal']['edit_articles'])>0){
			foreach($context['TPortal']['edit_articles'] as $bmg){
 				echo '
 								<option value="', $bmg['id']  ,'"' , $context['TPortal']['editmenuitem']['type']=='arti' && $context['TPortal']['editmenuitem']['IDtype']==$bmg['id'] ? ' selected' : ''  ,'> '.html_entity_decode($bmg['subject'], ENT_QUOTES, $context['characer_set']).'</option>';
			}
		}
		else
			echo '
	...
	global $context, $settings, $options, $txt, $scripturl, $modSettings, $boarddir, $boardurl, $language;
		...
		foreach($context['TPortal']['catnames'] as $ta => $val){
			...
			if(in_array($ta,$clist))
				echo ' checked';
			echo '>'.html_entity_decode($val, ENT_QUOTES, $context['characer_set']).'<br />';
		}
...
<div class="tborder article render1">
	<div class="windowbg2" style="margin: 0; padding: 0; overflow: hidden;">
		<h2 class="catbg article_title">{article_shortdate} {article_title} {article_category}</h2>
		{article_avatar}
		...
		<div class="article_padding">{article_text}</div>
			...
			echo '<br style="clear: both;" />
									<textarea id="tp_frontpage_template" name="tp_frontpage_template" style="width: 90%; height: 200px;">' .htmlspecialchars($context['TPortal']['frontpage_template'], ENT_QUOTES, $context['characer_set']). '</textarea>
					</td>
					</tr>
					<tr class="windowbg2">
	...
	<form accept-charset="', $context['character_set'], '" name="tpadmin_news" action="' . $scripturl . '?action=tpadmin" method="post" style="margin: 0px;">
		...
		<table class="admintable">
					...
					<td class="tborder" style="padding: 0; border: none;">
						<table class="formtable">
							<tr class="windowbg">
								...
								<td class="right"><input size="40" name="tp_category_value1" style="width: 90%;" type="text" value="' ,html_entity_decode($mg['value1'], ENT_QUOTES, $context['characer_set']), '"></td>
							</tr>
							<tr class="windowbg2">
			...
			foreach($context['TPortal']['editcats'] as $b => $parent)
			{
				if($parent['id']!= $mg['id'])
					echo '
										<option value="' . $parent['id'] . '"' , $parent['id']==$mg['value2'] ? ' selected="selected"' : '' , '>' , str_repeat("-",$parent['indent']) ,' ' , html_entity_decode($parent['name'], ENT_QUOTES, $context['characer_set']) , '</option>'; 
			}
			echo '
									</select>
								</td>
		...
		echo '			
							...
							<tr class="windowbg2">
								...
								<td class="right">
									...
									<a href="' . $scripturl . '?action=tpadmin;catdelete=' .$mg['id']. ';sesc='.$context['session_id'].'" onclick="javascript:return confirm(\''.$txt['tp-confirmcat1'].'  \n'.$txt['tp-confirmcat2'].'\')">
	...
	<div class="windowbg2" style="margin: 0; padding: 0; overflow: hidden;">
		<h2 class="catbg article_title">{article_shortdate} {article_title} {article_category}</h2>
			...
			echo '<br style="clear: both;" />
									...
									<textarea id="tp_category_value9" name="tp_category_value9" style="width: 90%; height: 200px;">' .htmlspecialchars($mg['value9'], ENT_QUOTES, $context['characer_set']). '</textarea>
								</td>
							</tr>
							<tr class="windowbg">
							...
							<tr class="windowbg2">
								...
								<td class="right">
									<table class="formtable">
                                        <tr class="windowbg2">							
    										<td width="50%">
                                                <input type="checkbox" name="tp_category_leftpanel" value="1"' , isset($mg['leftpanel']) ? ' checked="checked"' : '' ,' />', $txt['tp-displayleftpanel'] ,'</td>
    										<td>
                                                <input type="checkbox" name="tp_category_rightpanel" value="1"' , isset($mg['rightpanel']) ? ' checked="checked"' : '' ,' />', $txt['tp-displayrightpanel'] ,'</td>
    									</tr>
                                        <tr class="windowbg2">
    										<td><input type="checkbox" name="tp_category_toppanel" value="1"' , $mg['toppanel']==1 ? ' checked="checked"' : '' ,' />', $txt['tp-displaytoppanel'] ,'</td>
    										<td><input type="checkbox" name="tp_category_bottompanel" value="1"' , $mg['bottompanel']==1 ? ' checked="checked"' : '' ,' />', $txt['tp-displaybottompanel'] ,'<td/>
    									</tr>
                                        <tr class="windowbg2">
    										<td>
                                                <input type="checkbox" name="tp_category_upperpanel" value="1"' , $mg['upperpanel']==1 ? ' checked="checked"' : '' ,' />', $txt['tp-displayupperpanel'] ,'</td>
    										<td>
                                                <input type="checkbox" name="tp_category_lowerpanel" value="1"' , $mg['lowerpanel']==1 ? ' checked="checked"' : '' ,' />', $txt['tp-displaylowerpanel'] ,'</td>
    									</tr>
                                        <tr class="windowbg2">
    									</tr>
                                    </table>
		...
		foreach($context['TPortal']['arts'] as $a => $alink)
		{
			...
			echo '
								<tr class="windowbg2">
									...
									<td class="smalltext" align="center">
										<a href="' . $scripturl . '?action=tpadmin;sesc='.$context['session_id'].';artdelete=' .$alink['id'] , !empty($_GET['cu']) ? ';cu=' . $_GET['cu'] : '' , '" onclick="javascript:return confirm(\''.$txt['tp-articleconfirmdelete'].'\')">
										<img title="'.$txt['tp-delete'].'" border="0" src="' .$settings['tp_images_url']. '/tp-delete_shout.gif" alt="'.$txt['tp-delete'].'"  />
										</a>
									</td>
								</tr>';
			...
			<tbody>
				...
				<table class="multirow" width="100%">
					<tr class="windowbg2">
						...
						<td valign="top" width="80%"><input style="width: 92%;" name="tp_article_subject" type="text" value="'.htmlspecialchars_decode($mg['subject'], ENT_QUOTES, $context['characer_set']).'"></td>
					</tr>
					<tr class="windowbg2">
				...
				if($mg['articletype']=='php')
					echo '
							<textarea name="tp_article_body" id="tp_article_body" style="width: 95%; height: 300px;" wrap="auto">' , htmlentities($mg['body'], ENT_QUOTES, $context['characer_set']), '</textarea><br />';
				elseif($context['TPortal']['use_wysiwyg']>0 && ($mg['articletype']=='' || $mg['articletype']=='html'))
					TPwysiwyg('tp_article_body', htmlspecialchars($mg['body'], ENT_QUOTES, $context['characer_set']), true,'qup_tp_article_body', isset($context['TPortal']['editorchoice']) ? $context['TPortal']['editorchoice'] : false);
				elseif($context['TPortal']['use_wysiwyg']==0 && ($mg['articletype']=='' || $mg['articletype']=='html'))
					echo '
				...
				elseif($mg['articletype']=='bbc')
					TP_bbcbox('TPadmin3','tp_article_body', htmlspecialchars_decode(strip_tags($mg['body']), ENT_QUOTES, $context['characer_set']));
				else
				...
				echo '
					...
					<tr class="windowbg2">
						<td class="left" valign="top" align="right">'.$txt['tp-date'].'</td><td valign="top">';
				// day
				echo '
				...
				if($mg['articletype']=='php' || $mg['articletype']=='' || $mg['articletype']=='html')
				{
					...
					if($context['TPortal']['use_wysiwyg']>0 && ($mg['articletype']=='' || $mg['articletype']=='html'))
						TPwysiwyg('tp_article_intro', htmlspecialchars($mg['intro'], ENT_QUOTES, $context['characer_set']), true,'qup_tp_article_intro', true, false);
					else
						echo '
				...
				elseif($mg['articletype']=='bbc')
				{
					...
					<tr class="windowbg2">
						...
						<textarea name="tp_article_intro" id="tp_article_intro" style="width: 100%; height: 140px;" rows=5 cols=20 wrap="on">'.htmlspecialchars($mg['intro'], ENT_QUOTES, $context['characer_set']).'</textarea>
					</td></tr>';
				}
...
function template_blockedit()
{
	...
	$newtitle = htmlspecialchars_decode(TPgetlangOption($context['TPortal']['blockedit']['lang'], $context['user']['language']), ENT_QUOTES, $context['characer_set']);
		if(empty($newtitle))
	...
	<form accept-charset="', $context['character_set'], '" name="tpadmin_news" enctype="multipart/form-data" action="' . $scripturl . '?action=tpadmin" method="post" style="margin: 0px;">
		...
		<table class="admintable">
			...
			if($context['TPortal']['blockedit']['type']=='5' || $context['TPortal']['blockedit']['type']=='10' || $context['TPortal']['blockedit']['type']=='11')
			{
				...
				if($context['TPortal']['blockedit']['type']=='10')
				{
					echo '
						...
						<textarea style="width: 94%;" name="tp_block_body" id="tp_block_body" rows="15" cols="40" wrap="auto">' , htmlentities($context['TPortal']['blockedit']['body'], ENT_QUOTES, $context['characer_set']), '</textarea>
			...
			elseif($context['TPortal']['blockedit']['type']=='13'){
				...
				if(!in_array($context['TPortal']['blockedit']['body'],array('recentpoll','toppoll','topposters','topboards','topreplies','topviews','calendar')))
					...
					echo '<br /><input name="tp_block_body" type="radio" value="recentpoll" ' , $context['TPortal']['blockedit']['body']=='recentpoll' ? 'checked' : '' , '>'.$txt['tp-ssi-recentpoll'];
					echo '<br /><input name="tp_block_body" type="radio" value="toppoll" ' , $context['TPortal']['blockedit']['body']=='toppoll' ? 'checked' : '' , '>'.$txt['tp-ssi-toppoll'];
			...
			elseif($context['TPortal']['blockedit']['type']=='18'){
				...
				foreach($context['TPortal']['edit_articles'] as $art => $article ){
					echo '<option value="'.$article['id'].'" ' , $context['TPortal']['blockedit']['body']==$article['id'] ? ' selected="selected"' : '' ,' >'.html_entity_decode($article['subject'], ENT_QUOTES, $context['characer_set']).'</option>';
				}
				echo '</select>';
			}
			...
			elseif($context['TPortal']['blockedit']['type']=='19')
			{
				...
				foreach($context['TPortal']['catnames'] as $cat => $catname){
					echo '<option value="'.$cat.'" ' , $context['TPortal']['blockedit']['body']==$cat ? ' selected' : '' ,' >'.html_entity_decode($catname, ENT_QUOTES, $context['characer_set']).'</option>';
				}
			...
			foreach($context['TPortal']['langfiles'] as $langlist => $lang){
				if($lang!=$context['user']['language'] && $lang!='')
					echo '<input name="tp_lang_'.$lang.'" type="text" value="' , !empty($context['TPortal']['blockedit']['langfiles'][$lang]) ? html_entity_decode($context['TPortal']['blockedit']['langfiles'][$lang], ENT_QUOTES, $context['characer_set']) : html_entity_decode($context['TPortal']['blockedit']['title'], ENT_QUOTES, $context['characer_set']) , '"> '. $lang.'<br />';
			}
		...
		if($context['TPortal']['blockedit']['bar']!=4)
		{
			// extended visible options
							...
							<table class="admintable windowbg2">
								<tr class="windowbg2">
									<td width="50%" valign="top"><h4>Actions:</h4>
											...
											<input name="actiontype8" type="checkbox" value="profile" ' ,in_array('profile',$context['TPortal']['blockedit']['access2']['action']) ? 'checked="checked"' : '' , '>'.$txt[79].'<br />
											...
											<input name="actiontype10" type="checkbox" value="calendar" ' ,in_array('calendar',$context['TPortal']['blockedit']['access2']['action']) ? 'checked="checked"' : '' , '>'.$txt['calendar24'].'<br />
											<input name="actiontype11" type="checkbox" value="admin" ' ,in_array('admin',$context['TPortal']['blockedit']['access2']['action']) ? 'checked="checked"' : '' , '>'.$txt[2].'<br />
											<input name="actiontype12" type="checkbox" value="login" ' ,in_array('login',$context['TPortal']['blockedit']['access2']['action']) ? 'checked="checked"' : '' , '>'.$txt[34].'<br />
											<input name="actiontype13" type="checkbox" value="logout" ' ,in_array('logout',$context['TPortal']['blockedit']['access2']['action']) ? 'checked="checked"' : '' , '>'.$txt[108].'<br />
											<input name="actiontype14" type="checkbox" value="register" ' ,in_array('register',$context['TPortal']['blockedit']['access2']['action']) ? 'checked="checked"' : '' , '>'.$txt[97].'<br />
											<input name="actiontype15" type="checkbox" value="post" ' ,in_array('post',$context['TPortal']['blockedit']['access2']['action']) ? 'checked="checked"' : '' , '>'.$txt[105].'<br />
											...
											<input name="actiontype17" type="checkbox" value="search" ' ,in_array('search',$context['TPortal']['blockedit']['access2']['action']) ? 'checked="checked"' : '' , '>'.$txt[182].'<br />
											<input name="actiontype18" type="checkbox" value="mlist" ' ,in_array('mlist',$context['TPortal']['blockedit']['access2']['action']) ? 'checked="checked"' : '' , '>'.$txt['tp-memberlist'].'<br />';
				...
				{
					echo '
										<input type="checkbox" name="articletype' , $a , '" value="'.$bb['id'].'" ' ,in_array($bb['id'], $context['TPortal']['blockedit']['access2']['page']) ? 'checked="checked"' : '' , '>'.html_entity_decode($bb['subject'], ENT_QUOTES, $context['characer_set']).'<br />';
					$a++;
				}
				echo '
									</div></td>
			...
			echo '
					<tr class="catbg3">
						...
						<b>'.$txt['tp-'.$side[$i].'sideblocks'].'</b>
							<a href="'.$scripturl.'?action=tpadmin;addblock=' . $side[$i] . ';sesc='.$context['session_id'].'">
								&nbsp;&nbsp;[' , $txt['tp-addblock'] , ']
							</a>						
						</td>
					</tr>
			...
			if($tn>0)
			{
				foreach($context['TPortal']['admin_'.$side[$i].'block']['blocks'] as $lblock)
				{
					...
					echo '
						...
						<td colspan="2"><input name="title' .$lblock['id']. '" type="text" size="20" value="' .html_entity_decode($newtitle, ENT_QUOTES, $context['characer_set']). '"></td>
						<td>
			...
			if(function_exists('ctheme_tp_blocks'))
			{
				...
				echo '
					...
					switch($side[$i]){
						case 'left':
 							echo '
							<a href="' . $scripturl . '?action=tpadmin;sesc='.$context['session_id'].';blockright=' .$lblock['id']. '"><img title="'.$txt['tp-moveright'].'" src="' .$settings['tp_images_url']. '/TPselect_right.gif" alt="'.$txt['tp-moveright'].'" /></a>
							<a href="' . $scripturl . '?action=tpadmin;sesc='.$context['session_id'].';blockcenter=' .$lblock['id']. '"><img title="'.$txt['tp-movecenter'].'" src="' .$settings['tp_images_url']. '/TPselect_upper.gif" alt="'.$txt['tp-movecenter'].'" /></a>
							<a href="' . $scripturl . '?action=tpadmin;sesc='.$context['session_id'].';blockfront=' .$lblock['id']. '"><img title="'.$txt['tp-movefront'].'" src="' .$settings['tp_images_url']. '/TPselect_front.gif" alt="'.$txt['tp-movefront'].'" /></a>
							<a href="' . $scripturl . '?action=tpadmin;sesc='.$context['session_id'].';blockbottom=' .$lblock['id']. '"><img title="'.$txt['tp-movedown'].'" src="' .$settings['tp_images_url']. '/TPselect_bottom.gif" alt="'.$txt['tp-movedown'].'" /></a>
							<a href="' . $scripturl . '?action=tpadmin;sesc='.$context['session_id'].';blocktop=' .$lblock['id']. '"><img title="'.$txt['tp-moveup'].'" src="' .$settings['tp_images_url']. '/TPselect_top.gif" alt="'.$txt['tp-moveup'].'" /></a>
							<a href="' . $scripturl . '?action=tpadmin;sesc='.$context['session_id'].';blocklower=' .$lblock['id']. '"><img title="'.$txt['tp-movelower'].'" src="' .$settings['tp_images_url']. '/TPselect_lower.gif" alt="'.$txt['tp-movelower'].'" /></a>';
							break;
						case 'right':
 							echo '
							<a href="' . $scripturl . '?action=tpadmin;sesc='.$context['session_id'].';blockleft=' .$lblock['id']. '"><img title="'.$txt['tp-moveleft'].'" src="' .$settings['tp_images_url']. '/TPselect_left.gif" alt="'.$txt['tp-moveleft'].'" /></a>
							<a href="' . $scripturl . '?action=tpadmin;sesc='.$context['session_id'].';blockcenter=' .$lblock['id']. '"><img title="'.$txt['tp-movecenter'].'" src="' .$settings['tp_images_url']. '/TPselect_upper.gif" alt="'.$txt['tp-movecenter'].'" /></a>
							<a href="' . $scripturl . '?action=tpadmin;sesc='.$context['session_id'].';blockfront=' .$lblock['id']. '"><img title="'.$txt['tp-movefront'].'" src="' .$settings['tp_images_url']. '/TPselect_front.gif" alt="'.$txt['tp-movefront'].'" /></a>
							<a href="' . $scripturl . '?action=tpadmin;sesc='.$context['session_id'].';blockbottom=' .$lblock['id']. '"><img title="'.$txt['tp-movedown'].'" src="' .$settings['tp_images_url']. '/TPselect_bottom.gif" alt="'.$txt['tp-movedown'].'" /></a>
							<a href="' . $scripturl . '?action=tpadmin;sesc='.$context['session_id'].';blocktop=' .$lblock['id']. '"><img title="'.$txt['tp-moveup'].'" src="' .$settings['tp_images_url']. '/TPselect_top.gif" alt="'.$txt['tp-moveup'].'" /></a>
					...
					echo '
						...
						<td colspan="2" align="center">
							<a href="' . $scripturl . '?action=tpadmin;blockedit=' .$lblock['id']. ';sesc='.$context['session_id'].'"><img title="'.$txt['tp-edit'].'" border="0" align="middle" src="' .$settings['tp_images_url']. '/TPmodify.gif" alt="'.$txt['tp-edit'].'"  /></a>';
				...
				echo '	
	                    ...
	                    <td colspan="2" align="center">
							<a href="' . $scripturl . '?action=tpadmin;sesc='.$context['session_id'].';blockdelete=' .$lblock['id']. '" onclick="javascript:return confirm(\''.$txt['tp-blockconfirmdelete'].'\')"><img title="'.$txt['tp-delete'].'" align="middle" border="0" src="' .$settings['tp_images_url']. '/tp-delete_shout.gif" alt="'.$txt['tp-delete'].'"  /></a>
						</td>
					</tr>';
					...
					elseif($lblock['type']=='articlebox'){
						...
						foreach($context['TPortal']['edit_articles'] as $article){
							echo '
											<option value="'.$article['id'].'" ' ,$lblock['body']==$article['id'] ? ' selected' : '' ,' >'. html_entity_decode($article['subject'], ENT_QUOTES, $context['characer_set']).'</option>';
						}
						echo '
										</select>
									</td>
					...
					elseif($lblock['type']=='categorybox'){
						...
						if(isset($context['TPortal']['catnames']) && count($context['TPortal']['catnames'])>0)
						{
							...
							{
								echo '
											<option value="'.$cat.'" ' , $lblock['body']==$cat ? ' selected' : '' ,' >'.html_entity_decode($val, ENT_QUOTES, $context['characer_set']).'</option>';
							}
						}
						echo '
										</select>
