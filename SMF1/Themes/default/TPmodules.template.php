<?php
// Version: TinyPortal 1.0; TPmodules
	...
	if(isset($context['TPortal']['subaction'])){
		...
		<form accept-charset="', $context['character_set'], '"  name="TPadmin3" action="' . $scripturl . '?action=tpmod;sa=savearticle" method="post" enctype="multipart/form-data" onsubmit="syncTextarea();">
			<table width="100%" cellspacing="1" cellpadding="5" class="bordercolor">
				...
				{
					TP_bbcbox('TPadmin3','tp_article_body'. $mg['id'], htmlspecialchars_decode( $mg['body'], ENT_QUOTES, $context['characer_set']));
				}
				else
				...
				elseif($mg['articletype']=='bbc')
				{
					TP_bbcbox('TPadmin3','tp_article_intro'. $mg['id'], htmlspecialchars_decode( $mg['intro'], ENT_QUOTES, $context['characer_set']));
				}
		...
		<form accept-charset="', $context['character_set'], '"  name="TPadmin3" action="' . $scripturl . '?action=tpmod;sa=saveblock'.$context['TPortal']['blockedit']['id'].'" method="post">
			<table width="100%" cellspacing="1" cellpadding="5" class="bordercolor">
				...
				{
					TP_bbcbox('TPadmin3','blockbody' .$context['TPortal']['blockedit']['id'], htmlspecialchars($context['TPortal']['blockedit']['body'], ENT_QUOTES, $context['characer_set']));
				}
				elseif($context['TPortal']['blockedit']['type']=='10')
				{
					echo $txt['tp-body'].' <br /><textarea style="width: 94%;" name="blockbody' .$context['TPortal']['blockedit']['id']. '" rows=15 cols=40 wrap="auto">' .htmlspecialchars($context['TPortal']['blockedit']['body'], ENT_QUOTES, $context['characer_set']). '</textarea>';
				}
		...
		<div class="windowbg">
			...
			if(count($context['TPortal']['myarticles'])>0)
			{
				...
				elseif(allowedTo('tp_articles'))
					...
					if($art['off']==0 && $art['approved']==1)
						echo '
						<a href="' . $scripturl . '?page='.$art['id'].'">' . html_entity_decode($art['subject'], ENT_QUOTES, $context['characer_set']) . '</a>';
					else
						echo '
					' . html_entity_decode($art['subject'], ENT_QUOTES, $context['characer_set']);
