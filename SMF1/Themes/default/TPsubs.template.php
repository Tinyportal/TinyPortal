...
function TPortal_themebox()
{
	...
	if(isset($context['TPortal']['querystring']))
		$tp_where=htmlspecialchars(strip_tags($context['TPortal']['querystring']), ENT_QUOTES, $context['characer_set']);
	else
		...
		<form name="jumpurl1" onsubmit="return jumpit()" class="middletext" action="" style="padding: 0; margin: 0; text-align: center;">
         ...
         echo '
			...
			<input style="margin: 5px 0px 5px 10px;" type="button" value="'.$txt['tp-changetheme'].'" onclick="jumpit()" />
 			<input type="hidden" value="'.htmlspecialchars($scripturl . '?'.$tp_where.'theme='.$settings['theme_id'], ENT_QUOTES, $context['characer_set']).'" name="jumpurl3" />
...
function article_globaltags($render=true)
{
	...
	if(in_array('globaltags', $context['TPortal']['article']['visual_options']) && !empty($taglinks))
	{
		...
		foreach($taglinks as $tag)
				$code .= '
				<li><a href="' . $scripturl . $tag['href'] . '"' . ($tag['type']=='tparticle_itemtags' && $tag['itemid']==$context['TPortal']['article']['id'] ? ' class="selected"' : '') . '>' . strip_tags(html_entity_decode($tag['title'], ENT_QUOTES, $context['characer_set']), '<a>') . '</a></li>';
	...
	if(in_array('category',$context['TPortal']['article']['visual_options']))
	{
		...
		<div class="windowbg2 tp_pad">
			...
			foreach($context['TPortal']['article']['others'] as $art)
				$code .= '
				<li' . (isset($art['selected']) ? ' class="selected"' : '') . '><a href="' . $scripturl . '?page=' . (!empty($art['shortname']) ? $art['shortname'] : $art['id']) . '">' . html_entity_decode($art['subject'], ENT_QUOTES, $context['characer_set']) . '</a></li>';
			$code .= '
			</ul>
		</div>';
		}
