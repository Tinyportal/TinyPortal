...
function article_globaltags($render=true)
{
	...
	if(in_array('globaltags', $context['TPortal']['article']['visual_options']) && !empty($taglinks))
	{
		...
		foreach($taglinks as $tag)
				$code .= '
				<li><a href="' . $scripturl . $tag['href'] . '"' . ($tag['type']=='tparticle_itemtags' && $tag['itemid']==$context['TPortal']['article']['id'] ? ' class="selected"' : '') . '>' . strip_tags(html_entity_decode($tag['title']),'<a>') . '</a></li>';
