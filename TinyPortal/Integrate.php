...
class Integrate
{
	...
	public static function hookPermissions(&$permissionGroups, &$permissionList, &$leftPermissionGroups, &$hiddenPermissions, &$relabelPermissions)
	{
		$permissionList['membergroup'] = array_merge(
			[
				'tp_settings' => [false, 'tp', 'tp'],
				'tp_blocks' => [false, 'tp', 'tp'],
				'tp_articles' => [false, 'tp', 'tp'],
				'tp_submithtml' => [false, 'tp', 'tp'],
				'tp_submitbbc' => [false, 'tp', 'tp'],
				'tp_editownarticle' => [false, 'tp', 'tp'],
				'tp_alwaysapproved' => [false, 'tp', 'tp'],
				'tp_artcomment' => [false, 'tp', 'tp'],
				'tp_can_admin_shout' => [false, 'tp', 'tp'],
				'tp_can_shout' => [false, 'tp', 'tp'],
				'tp_dlmanager' => [false, 'tp', 'tp'],
				'tp_dlupload' => [false, 'tp', 'tp'],
				'tp_dlcreatetopic' => [false, 'tp', 'tp'],
				'tp_can_list_images' => [false, 'tp', 'tp'],
				'tp_can_search' => [false, 'tp', 'tp'],
			],
	...
	public static function hookBuffer($buffer)
	{
		...
		elseif (isset($context['TPortal']) && in_array($context['TPortal']['action'], ['search', 'search2'])) {
			$bodyid = 'searchpage';
		}
		...
		$string = '<a target="_blank" href="https://www.tinyportal.net" title="TinyPortal">TinyPortal ' . $tpversion . '</a> &copy; <a href="' . $scripturl . '?action=tportal;sa=credits" title="Credits">2005-2026</a>';
		...
		if (!in_array($context['current_action'], ['post', 'post2'])) {
			$finds[] = '[cutoff]';
			$replaces[] = '';
		}
	...
	public static function hookProfileArea(&$profile_areas)
	{
		...
		if (!$context['TPortal']['hideprofileoption'] == '1') {
			...
			$profile_areas['tp']['areas']['tpsummary'] = [
				'label' => $txt['tpsummary'],
				'file' => 'TPSubs.php',
				'function' => 'tp_summary',
				'icon' => 'menu_tp',
			...
			if (!$context['TPortal']['use_wysiwyg'] == '0') {
				$profile_areas['tp']['areas']['tparticles'] = [
					...
					'subsections' => [
						'articles' => [$txt['tp-articles'], ['profile_view_own', 'profile_view_any']],
						'settings' => [$txt['tp-settings'], ['profile_view_own', 'profile_view_any']],
					],
				];
			}
			else {
			...
			if (!empty($context['TPortal']['show_download'])) {
				$profile_areas['tp']['areas']['tpdownload'] = [
			...
			if (!$context['TPortal']['profile_shouts_hide']) {
				$profile_areas['tp']['areas']['tpshoutbox'] = [
	...
	public static function hookCredits()
	{
		...
		$context['copyrights']['mods'][] = '<a target="_blank" href="https://www.tinyportal.net" title="TinyPortal">TinyPortal ' . $context['TPortal']['version'] . '</a> by the TinyPortal team &copy; <a href="' . $scripturl . '?action=tportal;sa=credits" title="TinyPortal - Credits">2005-2026</a>';
	}

	public static function hookActions(&$actionArray)
	{
		$actionArray['tpadmin'] = ['TPortalAdmin.php',     'TPortalAdmin'];
		$actionArray['tportal'] = ['TPortal.php',          'TPortal'];
		$actionArray['tpshout'] = ['TPShout.php',          'TPShout'];

		if (TP_SMF21) {
			$actionArray['forum'] = ['BoardIndex.php', 'BoardIndex'];
		}
		else {
			$actionArray['forum'] = ['', 'SMF\\Actions\\BoardIndex::call'];
		}
	}
	...
	{
		$no_stat_actions = array_merge($no_stat_actions, ['tpshout' => true]);
	}
	...
	public static function hookSearchLayers()
	{
		...
		if ($context['TPortal']['action'] == 'search' && allowedTo(['tp_can_search'])) {
	...
	public static function hookPreLoadTheme(&$id_theme)
	{
		...
		elseif (!isset($_GET['page']) && !isset($_GET['action']) && !isset($_GET['board']) && !isset($_GET['topic'])) {
			if (($theme = cache_get_data('tpFrontTheme', 120)) == null) {
				...
				$request = $dB->db_query(
					...
					['name' => 'front_type', 'value' => 'single_page']
				);
				if ($dB->db_num_rows($request) > 0) {
					$dB->db_free_result($request);
					$request = $dB->db_query(
		...
		elseif (isset($_GET['action']) && $_GET['action'] == 'tportal' && isset($_GET['dl'])) {
