...
function TP_loadTheme()
{
    ...
    elseif(!isset($_GET['page']) && !isset($_GET['action']) && !isset($_GET['board']) && !isset($_GET['topic']))
	{
		...
		if(tpdb_num_rows($request)>0)
		{
			$row=tpdb_fetch_row($request);
			tpdb_free_result($request);
			$request =  tp_query("SELECT art.id_theme as ID_THEME, sett.value FROM (" . $tp_prefix . "articles AS art, " . $tp_prefix . "settings AS sett) WHERE sett.name='featured_article' AND sett.value=art.id ", __FILE__, __LINE__);
			if(tpdb_num_rows($request)>0)
			{
				$row=tpdb_fetch_row($request);
				$theme=$row[0];
...
{
	global $maintenance, $tp_prefix, $db_prefix, $context, $scripturl,$txt , $user_info, $settings , $modSettings, $boarddir, $boardurl, $sourcedir, $forum_version;
...
function doTPpage()
{
	global $db_prefix, $tp_prefix, $context, $scripturl,$txt , $user_info, $settings , $modSettings, $boarddir, $boardurl, $sourcedir;
	...
	if(!empty($_GET['page']))
	{
		...
		if(!empty($page))
		{
			...
			if(allowedTo('tp_articles'))
				$request =  tp_query("SELECT art.*, art.author_id as authorID, art.id_theme as ID_THEME, var.value1,var.value2,var.value3,var.value4, var.value5,var.value7,var.value8, art.type as rendertype, 
			...
			if(tpdb_num_rows($request)>0)
			{
				...
				{	
					...
					$now = time();
					$request = tp_query("SELECT item FROM " . $tp_prefix . "data WHERE id_member=" . $context['user']['id'] . " AND type=1 AND value=" . $article['id'] . " LIMIT 1", __FILE__, __LINE__);
					if(tpdb_num_rows($request)>0)
					{
						$row=tpdb_fetch_row($request);
						$last=$row[0];
						tpdb_free_result($request);
						$request =  tp_query("UPDATE " . $tp_prefix . "data SET item=" . $now . " WHERE id_member=" . $context['user']['id'] . " AND type=1 AND value=" . $article['id'], __FILE__, __LINE__);
					}
					else
					{
						$last = $now;
						$request = tp_query("INSERT INTO " . $tp_prefix . "data (type, id_member, value, item) VALUES (1," . $context['user']['id'] . ", " . $article['id'] . ", '" . $now. "')", __FILE__, __LINE__);
					}
					...
					WHERE author_id = " . $context['TPortal']['article']['authorID'] . "
					...
					// fetch any comments
					$request =  tp_query("SELECT var.* , IFNULL(mem.real_name,0) as realName,mem.avatar,
		...
		if(!empty($cat))
		{
			...
			if(tpdb_num_rows($request)>0)
			{
				...
				if(get_perm($category['value3']))
				{
					...
					$request =  tp_query("SELECT COUNT(*) FROM " . $tp_prefix . "articles as art 
					WHERE art.category=" . $category['id'] . " 
					AND ((art.pub_start=0 AND art.pub_end=0) OR (art.pub_start!=0 AND art.pub_start<".$context['tportal']['now']." AND art.pub_end=0) OR (art.pub_start=0 AND art.pub_end!=0 AND art.pub_end>".$context['tportal']['now'].") OR (art.pub_start!=0 AND art.pub_end!=0 AND art.pub_end>".$context['tportal']['now']." AND art.pub_start<".$context['tportal']['now']."))
					AND art.off=0 AND art.approved=1", __FILE__, __LINE__);
					if(tpdb_num_rows($request)>0)
					{
						$row=tpdb_fetch_row($request);
						$all_articles=$row[0];
...
function doTPfrontpage()
{
	...
	elseif(in_array($context['TPortal']['front_type'], array('forum_articles', 'forum_selected_articles')))
	{
		...
		if(count($mposts)>0)
			$request =  tp_query("
			SELECT 
			m.subject, " . ($context['TPortal']['frontpage_limit_len']>0 ? "LEFT(m.body," . $context['TPortal']['frontpage_limit_len'] . ") as body" : "m.body") . ", IFNULL(mem.real_name, m.poster_name) AS realName, m.poster_time as date, mem.avatar,mem.posts, mem.date_registered as dateRegistered,mem.last_login as lastLogin,
				IFNULL(a.id_attach, 0) AS ID_ATTACH, a.filename, a.attachment_type as attachmentType, t.id_board as category, b.name as category_name,
				t.num_replies as numReplies, t.id_topic as id, m.id_member as authorID, t.num_views as views,t.num_replies as replies, t.locked,
				IFNULL(thumb.id_attach, 0) AS thumb_id, thumb.filename as thumb_filename
				FROM (" . $db_prefix . "topics AS t, " . $db_prefix . "messages AS m)
...
{	
	...
	if($context['user']['is_logged'])
	{
		...
		$result =  tp_query("SELECT type,value, item FROM " . $tp_prefix . "data WHERE type=2 AND id_member=" . $context['user']['id'], __FILE__, __LINE__);
